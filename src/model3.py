...
class E3GCL(MessagePassing):
    ...
    """
    def __init__(self, node_attr_dim, edge_dim=0, aggr='add', equivariant=False):
        ...
        self.mlp_e = nn.Sequential(
            nn.Linear(2 * node_attr_dim + 1 + edge_dim, node_attr_dim),
            # self.dropout,
            nn.SiLU(),
        ...
        self.mlp_x = nn.Sequential(
            ...
            # self.dropout,
            nn.SiLU(),
            nn.Linear(node_attr_dim, 1)
        )

        self.mlp_h = nn.Sequential(
            nn.Linear(2 * node_attr_dim, node_attr_dim),
            # self.dropout,
            nn.SiLU(),
            nn.Linear(node_attr_dim, node_attr_dim)
        )
...
class E3GG(nn.Module):
    ...
    """
    def __init__(self, node_attr_dim, edge_dim, num_layers=4, aggr='add', equivariant=False):
        super().__init__()

        self.GCL = nn.ModuleList()
        for layer in range(num_layers):
            self.GCL.append(E3GCL(node_attr_dim = node_attr_dim,
                                  edge_dim = edge_dim,
                                  aggr = aggr,
