...
from torch_geometric.nn import global_add_pool  # или global_mean_pool
...
class EquivariantGNN(torch.nn.Module):
    ...
    def forward(self, node_features, edge_indices, edges_features, batch_vec=None):
        """
        :param node_features: Node features, `num_nodes` x `input_node_dim` tensor.
        :param edge_indices: Edge node indices: two tensors (`first_node_ids`, `second_node_ids`)
        :param edges_features: Edge features, `num_edges` x `input_edge_dim` tensor.
        ...
        if self.predict_type == "coord":
            ...
            node_features = self.unembedding(node_features)  # [N_total, out_dim]
            if batch_vec is None:
                raise ValueError("Provide batch_vec (PyG batch.batch) for node_pooling.")
            graph_features = global_add_pool(node_features, batch_vec)  # [B, out_dim]
            pred = self.pool(graph_features)  # [B, 1]
        else:
            raise ValueError("Unknown predict type.")
